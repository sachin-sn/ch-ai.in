...
export default function ArchDiagram() {
  return (
    <svg viewBox="0 0 720 330" role="img" aria-label="Architecture diagram: a git push flows through GitHub Actions, authenticated via Workload Identity Federation, into Firebase Hosting on GCP; a visitor's browser looks up ch-ai.in in Cloud DNS and is served the static site from Firebase Hosting.">
      <defs>
      ...
      <g style={{ fontFamily: "var(--sc-font-mono)" }}>
        ...
        <text x="100" y="155" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">WIF token exchange —</text>
        <text x="100" y="168" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">no stored GCP keys</text>
        ...
        <text x="562" y="0" textAnchor="middle" fontSize="10.5" fill="var(--sc-ink-dim)" transform="translate(0,12)">GCP — Cloud DNS + Firebase Hosting</text>

        <rect x="440" y="34" width="244" height="50" rx="6" fill="var(--sc-bg-panel-raised)" stroke="var(--sc-line)" />
        <text x="562" y="55" textAnchor="middle" fontSize="11.5" fontWeight="700" fill="var(--sc-ink)">Cloud DNS</text>
        <text x="562" y="71" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">managed zone for ch-ai.in</text>

        <rect x="440" y="102" width="244" height="50" rx="6" fill="var(--sc-bg-panel-raised)" stroke="var(--sc-line)" />
        <text x="562" y="123" textAnchor="middle" fontSize="11.5" fontWeight="700" fill="var(--sc-ink)">Firebase Hosting</text>
        <text x="562" y="139" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">site: ch-ai-portfolio, global CDN</text>

        <rect x="440" y="170" width="244" height="50" rx="6" fill="var(--sc-bg-panel-raised)" stroke="var(--sc-line)" />
        <text x="562" y="191" textAnchor="middle" fontSize="11.5" fontWeight="700" fill="var(--sc-ink)">CDKTF (Terraform CDK)</text>
        <text x="562" y="207" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">provisions the two boxes above</text>

        <rect x="440" y="238" width="244" height="56" rx="6" fill="var(--sc-bg-panel-raised)" stroke="var(--sc-line)" />
        <text x="562" y="260" textAnchor="middle" fontSize="11.5" fontWeight="700" fill="var(--sc-ink)">applied by hand</text>
        <text x="562" y="276" textAnchor="middle" fontSize="10" fill="var(--sc-ink-dim)">no infra CI yet — npx cdktf deploy</text>
        ...
        {/* visitor -> cloud dns -> firebase hosting (request path) */}
        <path d="M184 268 C 320 268, 320 59, 440 59" fill="none" stroke="var(--sc-ink-dim)" strokeWidth="1.3" strokeDasharray="2 3" markerEnd="url(#sc-arrow-dim)" />
        <path d="M562 84 L562 102" fill="none" stroke="var(--sc-ink-dim)" strokeWidth="1.3" strokeDasharray="2 3" markerEnd="url(#sc-arrow-dim)" />
        <text x="196" y="316" fontSize="9.5" fill="var(--sc-ink-dim)">DNS lookup resolves to Firebase Hosting&apos;s</text>
        <text x="196" y="328" fontSize="9.5" fill="var(--sc-ink-dim)">anycast IPs, then an HTTPS GET is served from its CDN</text>
      </g>
    </svg>
  );
}
